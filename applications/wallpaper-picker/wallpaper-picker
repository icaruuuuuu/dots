...
# Props to Rliop913 on Github for making this marvelous script 
...
selected=$(echo -en "${strrr}" | rofi -dmenu -no-layers -p " " -config ~/.config/rofi/wallpaper-picker.rasi )
awww img "${bgresult[$selected]}" --transition-type center --transition-duration 0.5
sleep 0.2
wallust run -s "${bgresult[$selected]}"
